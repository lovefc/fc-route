 ...
 * @LastEditTime : 2022-02-17 09:58:04
 */
...
class route {
	...
	constructor() {
		...
		this.staticPage = function (req, res, fileURL) {
			_fs.readFile(fileURL, function (err, data) {
				//读完之后做的事情
				if (err) {
					//文件不存在
					this.notPage(req, res);
				}
				res.writeHead(200, {
					"Content-Type": mime
				});
		...
		for (let i = 0; i < this.methods.length; i++) {
			...
			this[method] = function (rule, callback) {
				if (this.isFun(rule)) {
					that.routeRule[method][i] = rule;
				} else {
					rule = this.prefixStr + rule;
	...
	match(rulestr, routePath) {
		...
		//let rulestr = new RegExp(rulestr);
		// 如果在规则中没遇到一些特殊字符串
		if (rulestr.search(/(%|@|:|~|!)/) == -1) {
			if (route == rulestr) {
				return true;
		...
		if (route && match) {
			...
			if (arr) {
				...
				this.params = param;
				//this.req.get = Object.assign(this.req.get, param);
				return true;
			}
		}
		return false;
